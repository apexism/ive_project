...
def process_frame(frame):
    ...
    roi = frame[roi_top:, :]
    left_line= width // 2 - 100
    right_line = width // 2 + 100
    ...
    if blue_contours:
        ...
        if M["m00"] != 0 :
                # cx = int(M["m10"] / M["m00"])
            cx = int(M["m10"] / M["m00"]) #
            if cx >= left_line and cx <= right_line:
                cv2.drawContours(roi, [max_contour], -1, (255, 0, 0), 2)
                
            center_line = width // 2
            if cx < center_line - 30:
                return "LEFT"
            elif cx > center_line + 30:
                return "RIGHT"
            else :
                return "GO"
        else:
...
def camera_thread():
    ...
    while True:
        ...
        if result:
            print(result)
            if result == "LEFT":
                if timer > 0 and time <= 0.1:
                    pub_thread.update(1, 0, 0, 0, 0.1, 0.00)
                elif timer > 0.1 and time <= 0.2:
                    pub_thread.update(1, 0, 0, 1, 0.00, 0.01)
                elif timer > 0.2 and time <= 0.3:
                    pub_thread.update(1, 0, 0, 0, 0.1, 0.00)
                elif timer > 0.3 and time <= 0.4:
                    pub_thread.update(1, 0, 0, 1, 0.00, 0.01)
                else:
                    set_time, timer = 0, 0
                ...
                pub_thread.update(1, 0, 0, 0, 0.2, 0.00)
                set_time, timer = 0, 0
